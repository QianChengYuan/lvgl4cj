...
v0.3 评审指出：v0.3 的方向（把 V1/V2 设为硬阻塞点）是对的，但**修订动作快于验证**，有几处是"用一个新的未验证假设替换了旧的未验证假设"。v0.4 **只做四件一等修订 + 一批次生修订**，其余延后到 V1/V2 出结论后一次性处理。

### 前提纠正：V2 的立论基础

v0.2 从 CJQT6 的 `setOnTimeoutCapture` 推出"仓颉可能原生支持捕获回调"，v0.3 将其作为分支 A 的依据。

**这个推论方向是反的。** `setOnTimeoutCapture` 是 CJQT6 自建闭包表的对外 API——它的存在**恰恰暗示仓颉默认不能直接捕获**，否则不需要专门的 `Capture` 变体。

**修订**：
- v0.4 将**分支 B（自建闭包表 + closure_id）定为默认设计**
- 分支 A 降为备选，**且必须满足 GC 保活前提**
- V2 验证拆为两步：**(1) 语言层能否编译捕获闭包**（5 分钟）+ **(2) C 侧能否长期持有**（GC 保活验证，关键）

### I 类：一等缺陷（必改）

| #    | 问题                                                         | 修订位置                                                |
| ---- | ------------------------------------------------------------ | ------------------------------------------------------- |
| I1   | 分支 A 遗漏 GC 保活；错误声称"不需要注销闭包"                | **§3.2 重写**：默认分支 B；分支 A 补 pin 表；删错误陈述 |
| I2   | anim var 二元矛盾（`var = obj` vs `var = closure_id` 不可能同时成立） | **§3.10 重写**：改为 `var = anim_ctx`；补取舍表         |
| I3   | flush_wait_cb 引入新死锁路径（与 postAndWait 双向等待；窗口关闭永久阻塞） | **§8.1.2 + §3.8.4**：超时 + 等待图检测                  |
| I4   | `lv_obj_clean` 判断方法不可实现                              | **§3.3**：显式标志 + 嵌套保护                           |

### J 类：次生问题

| #    | 问题                                            | 修订位置                                         |
| ---- | ----------------------------------------------- | ------------------------------------------------ |
| J1   | DEFERRED_LOOP break 后泄漏                      | §3.8.2：保留到下一帧继续 drain                   |
| J2   | post 异常语义不一致（入队失败 vs 任务执行失败） | §3.9.1：两张对照表                               |
| J3   | 方案 C 下线程检查失效                           | §3.6.5：`g_lvgl_thread_id` = 创建 runtime 的线程 |
| J4   | 方案 C 性能指标不可达                           | §11.2-C：改为条件式                              |
| J5   | 缺 V6（SDL2 flush_wait 线程关系验证）           | §12 + §8.1.2                                     |
| J6   | FFI ≤2μs 无依据                                 | §11.2：标注"待实测校准"                          |
| J7   | send_event param 限定太窄                       | §5.6：支持句柄 + 标量两条路径                    |
| J8   | buf_lines 与渲染模式约束未说明                  | §5.3：PARTIAL 有效，DIRECT/FULL 忽略             |
| J9   | P0 门禁"线程亲和性"表述歧义                     | §11.1：改为条件式                                |
| J10  | 速查卡 V2 缺第三选项                            | 附录 D：加"支持捕获但需显式保活"                 |

### 延后项

F/G/H 类（API 补齐、工程化细节）**全部延后**到 V1/V2 出结论后一次性处理。
...
⚠️ **v0.4 关键立场**：这四件事中的前两件（线程、回调）**依赖 P0 第 1 周的验证结论 V1/V2**。V1/V2 出结论前，§3.2 / §3.6 / §3.8 / §3.9 / §3.10 的**代码不应动工**，只做设计准备。
...
| 分支                             | 机制                                        | 定位         | 采用条件                                                     |
| -------------------------------- | ------------------------------------------- | ------------ | ------------------------------------------------------------ |
| **B（自建闭包表 + closure_id）** | 闭包存入全局表，`user_data` 装 `int32_t` ID | **默认设计** | V2-a 失败，**或** V2-b 失败，**或** 无明确 GC pin API        |
| A（直接持有闭包指针 + pin 表）   | `user_data` 装闭包指针，注册时 pin          | 备选         | V2-a 通过 **且** V2-b 通过 **且** 仓颉运行时提供明确的 pin API |
...
static void *g_pinned_closures[MAX_PINNED];
static int   g_pinned_count = 0;

void *lvglcj_pin_closure(void *closure) {
    // 加入全局根表，使其不被 GC 回收
    // 具体机制取决于仓颉运行时提供的 API：
    //   - 若有 GCHandle / GlobalRef 等价物，用它
    //   - 若无，可能需遍历根扫描或依赖特殊注解
    g_pinned_closures[g_pinned_count++] = closure;
    return closure;
}

void lvglcj_unpin_closure(void *closure) {
    for (int i = 0; i < g_pinned_count; i++) {
        if (g_pinned_closures[i] == closure) {
            g_pinned_closures[i] = g_pinned_closures[--g_pinned_count];
            return;
        }
    }
}
...
#### 3.3.2 `lv_obj_clean()` 语义（显式标志实现）★修订 I4

**问题**：v0.3 说 clean 时"通过事件目标的父子关系判断"，**不可实现**——`lv_obj_clean` 内部对每个子对象调 `lv_obj_delete`，每个子的 DELETE 事件的 target 就是那个子本身；从单个事件无法知道"它的父是不是正在被 clean"，因为父对象此刻仍活着且完全有效。
...
#### 3.6.5 线程断言（两套宏，Release 保留）★修订 J3
...
**★方案 C 下的语义修正（修订 J3）**：方案 C 没有独立 LVGL 线程。此时 `g_lvgl_thread_id` **应初始化为"创建 runtime 的线程 ID"**，即调用 `runtime.start()` 的那条线程。检查逻辑不变——方案 C 下"跨仓颉线程调 `pump()`"会被正确拦截。
...
void lvglcj_drain_deferred(void) {
    ...
    while (!lvglcj_deferred_empty(&g_deferred)) {
        if (++iter > MAX_ITER) {
            // ★ 修订 J1：不 break 丢弃，保留到下一帧继续 drain
            lvglcj_record_error(LVGLCJ_ERR_DEFERRED_LOOP, 0, __func__);
            return;   // 剩余项保留在 g_deferred 中
        }
...
**语义明确**（写入用户文档）：**回调中调 `obj.close()` 不会立即生效，但对象立刻进入"待删除"状态；实际删除最迟在同一帧内完成**。极端链式场景下若超过 64 轮，剩余项**保留到下一帧继续处理，不丢弃、不泄漏**。
...
#### 3.10.2 var 的二元矛盾（v0.3 未修干净）★修订 I2

**问题**：`var` 就是 `exec_cb` 收到的第一个参数，它**只能有一个值**。v0.3 同时提供了：

- §3.10.2 `lvglcj_anim_set_var(anim, obj)` ← `var = obj`
- §3.10.3 "把 var 直接用作 closure_id" ← `var = closure_id`
...
**修订方案：`var = anim_ctx`**（推荐）

```c
typedef struct {
    int64_t obj_handle;    // 动画目标对象
    int32_t closure_id;    // 仓颉闭包
    int64_t anim_handle;   // 反向引用，用于清理
} lvglcj_anim_ctx_t;
...
**代价补偿**：**失去 LVGL 原生的"对象删除自动停动画"**。在 DELETE 钩子里手动补：
...
static void lvglcj_delete_hook(lv_event_t *e) {
    ...
    // ... 原有 (a)(b)(c) ...
    // ★ (c) 停止该 obj 的所有动画
    lvglcj_stop_all_anims_of_obj(h);   // 遍历 obj → [anim] 列表，逐个 lv_anim_delete + free ctx
    // ... (d) ...
}
```
...
int32_t lvglcj_anim_set_target(int64_t anim, int64_t obj);        // ★ 改名：target 语义
...
int32_t lvglcj_anim_set_deleted_cb(int64_t anim, int32_t cid);
...
int64_t lvglcj_event_get_param_scalar(int64_t evh);    // ★ 新增 J7
int32_t lvglcj_event_stop_bubbling(int64_t evh);
int32_t lvglcj_event_stop_trickling(int64_t evh);
...
```c
int64_t lvglcj_anim_create(void);
int32_t lvglcj_anim_set_target(int64_t a, int64_t obj);      // ★ 改名
...
#### 8.1.2 线程关系与 flush_wait 同步 ★修订 I3
...
| `anim_test`       | 动画执行、停止、对象删除时清理；**同 obj 多动画各自正确回调** |
| `deferred_test`   | 回调中 `close()` 不崩，**同帧内**删除生效；**链式删除不泄漏** |
...
| 指标                | 门槛                                                      | 测量方式 |
| ------------------- | --------------------------------------------------------- | -------- |
| **线程亲和性**      | **若走方案 B 则必须 = 0；方案 A 下此指标 N/A**（修订 J9） | §3.6.4   |
...
⚠️ **方案 C 下 UI 与业务抢同一线程**。以下指标**条件式**（修订 J4）：
...
| V2     | 仓颉 FFI 回调是否支持捕获（§3.2.1）               | **拆两步：V2-a 编译 + V2-b 保活** |
| V3     | 亲和性探针测试（§3.6.4）                          | 实测数据                          |
| V4     | ASan 与仓颉 GC 兼容性（§9.5）                     | 内存检测方案 + suppression 规则   |
| V5     | cjpm `[ffi.c]` 字段可用性                         | 照抄 CJQT6 模板验证               |
| **V6** | **SDL2 flush_wait 同步与线程关系验证**（新增 J5） | 线程关系 + 超时行为结论           |

⚠️ **这六项不确定，P0 后续工作无法正确设计。**
...
配套前置：§12 的 V1–V6 六项不确定项结论。

**这七条过了，项目成立；过不了，先修设计，不要往前堆控件。**
...
| **pin 表**             | 分支 A 下保存闭包指针的全局根表，防止 GC 回收                |
| **anim_ctx**           | 动画执行上下文，存 `var` 指针，含 obj/closure_id/anim_handle |
...
| -15     | `DEFERRED_LOOP`     | 延迟删除循环超限（保留到下一帧） | `LvglException(DeferredLoop)`     |
| **-16** | **`PIN_EXHAUSTED`** | **pin 表满（分支 A）**           | `LvglException(PinExhausted)`     |
| **-17** | **`ANIM_CTX_LOST`** | **动画上下文丢失（异常状态）**   | `LvglException(AnimCtxLost)`      |
...
### D.1 第 1 周必须回答的 6 个问题 ★修订 J5/J10
...
| **V6**   | SDL2 flush_wait 与主线程渲染同步可行吗？ | 写最小 SDL2 双线程例子，验证信号量 + 超时       | ⬜ 可行 / ⬜ 需调整方案                                        |
...
| 章节                 | 为什么必读                              |
| -------------------- | --------------------------------------- |
| **§3.2 回调桥接**    | 分支 B 默认；分支 A 的 pin 表机制       |
| **§3.6 线程模型**    | 方案 A/C 选型直接决定代码结构           |
| **§3.8 重入 + 死锁** | 延迟删除 + 等待图检测是 P0 压力测试核心 |
| **§3.10 动画**       | anim_ctx 方案与手动停动画               |
| **§3.4 绘制缓冲**    | 缓冲所有权是花屏/段错误的最大来源       |

### D.3 第一个可交付物

见 §17：`examples/hello_cj` + 七个断言。
...
> v0.4 相对 v0.3 的核心变化：
> 1. **纠正 V2 立论**：默认走分支 B；分支 A 补 GC 保活（pin 表）+ 删错误陈述
> 2. **解决 anim var 二元矛盾**：改为 `var = anim_ctx`
> 3. **flush_wait_cb 强制超时** + 死锁等待图检测
> 4. **clean 语义显式标志** + 嵌套保护
> 5. 一批次生修订（J 类）
>
> 最重要的立场：**§3.2 / §3.6 / §3.8 / §3.9 / §3.10 的代码在 V1/V2 出结论前不动工**。这不是保守，而是避免"用一个新的未验证假设替换旧的未验证假设"。