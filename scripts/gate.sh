       ...
       cmake --build "${BUILD_DIR}" -j "$(nproc)" >> "${TMPDIR_GATE}/cmake.log" 2>&1; then
        if ctest --test-dir "${BUILD_DIR}" --output-on-failure > "${TMPDIR_GATE}/ctest.log" 2>&1; then
            ...
            if [[ "${N}" != "10" || "${TGT}" -lt "10" ]]; then
                ...
                # ★★ 顺带把"C 项检查"的**合计数**算出来并打印。
                #
                #   这个数字此前没有任何脚本产出，全靠人手工求和，于是出了两个问题：
                #     · 口径不一致：README 写 684，而按文档记录的方法实测是 701；
                #       README 的平台表里还有 381 / 345 / 332 三个数字，彼此也对不上
                #       （它们是各自那一时刻的快照，没有注明）。
                #     · 逐项明细没有留档，差异一旦出现就无法归因。
                #   现在由门禁直接给出：可复现、口径固定。
                #
                #   取法与 docs/benchmarks/2026-09-20_P0_gate.md 记录的一致：跑每个
                #   可执行、取最后一行含「项检查」的输出。★ 两种前缀都要吃 ——
                #   `=== 结果：N 项检查` 与 `=== PASS：N 项检查`（后者是 handle_table），
                #   早期手工求和漏掉的正是它。
                #   代价：这些用例会被再跑一遍（实测合计约半分钟，可接受）。
                TOTAL=0
                for t in "${BUILD_DIR}"/bin/test_*; do
                    [[ -x "${t}" ]] || continue
                    c="$(SDL_VIDEODRIVER=dummy timeout 900 "${t}" 2>&1 \
                         | grep -E '项检查' | tail -1 | grep -oE '[0-9]+' | head -1)"
                    TOTAL=$(( TOTAL + ${c:-0} ))
                done
                ok "C 单测全绿（${N} 个可执行，合计 ${TOTAL} 项检查）"
            fi
        else
            bad "C 单测有失败"
