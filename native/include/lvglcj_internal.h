...
/*
 * ============================================================================
 * 标准库头：放在这里，而不是让每个 .c 各自包含
 * ============================================================================
 * 理由不是省事，是一条**反复出现的教训**：本项目已经三次撞上同一类问题 ——
 * 某个文件新用上 malloc/free/calloc/strcpy/memset 却没包含对应头，
 * 而 gcc **只给一条告警、编译照样通过**：
 *
 *   warning: implicit declaration of function 'calloc'
 *
 * 危险性在于隐式声明的返回值被当作 int —— 在 64 位上 **会截断指针**。
 * 也就是说：一条被忽略的告警 = 一个只在大分配量下才现形的悬空指针。
 * 三次分别发生在 native/src/canvas.c、native/src/widgets.c、
 * test/native/test_style_widgets.c。
 *
 * 把这两个头放进内部头，就等于取消了"忘记包含"这种可能 ——
 * 所有 C 实现与 C 用例都包含本文件，因此它们自动获得正确的声明。
 * （各 .c 里若已有各自的 stdlib/string 包含也无妨，重复包含有 include guard。）
 * ============================================================================
 */
#include <stdlib.h>
#include <string.h>
